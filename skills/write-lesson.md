...
6. **Make beauty serve attention.** Composition, color, motion, and scale should signal
   the current teaching target. Remove interesting but irrelevant details.
...
Inspect the rendered lesson and learn what the platform actually supports. Do not infer
capabilities from desired prose or bypass a structured contract with text parsing.
