...
## Architecture
- `src/bootstrap.js` loads and validates the checked-in lesson/catalogs, stages bounded local Markdown paste/file imports without persistence, and activates only complete validated lessons through the same presentation/controller/renderer path after explicit user opening. It projects an optional unnumbered `entryScene` into a pre-scroll topic view, renders semantic prose, explicit lesson lifecycle status, declared supplementary HTTPS images, and scene fidelity records, owns scroll/explicit navigation and responsive disclosure state, probes WebGL2, and dynamically imports `src/main.js` only when rendering is available. It also owns the one native full-viewport Explore dialog and guarded lifecycle: reparent the existing stage/panel/disclosure, preserve independent scroll/focus locks, and restore the exact authored scene, lesson scroll, and invoking focus on Return/Escape. Imported images remain semantic DOM figures—not WebGL textures—and must disclose hosts before activation, load without referrers only after activation, retain complete caption/credit/source metadata and accessible failure recovery, and never permit scripts, styles, frames, arbitrary fetches, or undeclared media. `html`/`body` do not scroll in Lesson mode: the named focusable `#page-scroll` below the fixed topbar is the sole vertical lesson surface, so scene thresholds, explicit navigation, keyboard/touch reading, and compact disclosure restoration must use its coordinates. The native vertical scrollbar remains; horizontal travel is forbidden. The no-WebGL path must remain a complete readable lesson, preserve lifecycle identity and supplementary figures, hide renderer-only Explore actions, and must not fetch Three.js. Lifecycle `Draft` is not a geometry/activity fidelity category and must never be presented as one.
- `src/main.js` owns the one Three.js scene and exports the renderer-readiness/lesson-adapter boundary. Lesson and Explore state reaches it only as complete canonical snapshots—never prose or synthetic DOM events. Its Explore wrapper may expose the actual rendered camera, stable catalog-to-renderer panel projection, temporary command callback, reset pose, and semantic camera actions; do not add another renderer adapter or filter path. Camera aspect must follow the stage's exact CSS rectangle before and after reparenting; never compensate responsive framing with nonuniform scene, model, or canvas scaling.
...
- Visibility: canonical lesson snapshots are authoritative in Lesson mode. Explicit Explore creates one temporary complete canonical snapshot, projects it into the retained Viewer controls by stable entity ID, synchronizes the actual rendered camera before allowlisted command batches, and discards all edits on Return. Lesson mode must disable the fieldset; Explore grants full temporary orbit/zoom/pan/filter control without another canvas, context, transform, or filter system. Geometry/activity status and source detail belong in **Model & sources**; do not duplicate them as in-canvas badges, persistent stage rows, or global progress/status chrome.
