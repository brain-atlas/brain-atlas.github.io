...
- `cmd/brain-atlas`, `internal/site`, `internal/standalone`, and `src/standalone` form the optional workstation-binary boundary. Read `internal/standalone/SPEC.md` before changing it. The ordinary Pages build must contain no lifecycle client. The standalone build embeds the reviewed publication artifact, uses only the Go standard library with `CGO_ENABLED=0`, accepts loopback addresses only, authenticates held page-lifecycle requests with a process-random strict HttpOnly cookie, and never treats the browser-launch helper as browser lifetime. Do not add LAN serving, a native WebView/tray, cgo, runtime adjacent-file loading, or a general local API without a separate approved security/design change.
...
CGO_ENABLED=0 go test ./...         # standalone server/embed/launcher tests
```
