...
- Visibility: canonical lesson snapshots are authoritative in Lesson mode; the collapsed legacy Viewer-controls panel retains `sceneState.visible`, layer pills, and filters for later Explore mode. A scene control policy must disable that fieldset when direct mutations are not allowed. Geometry/activity status and source detail belong in **Model & sources**; do not duplicate them as in-canvas badges, persistent stage rows, or global progress/status chrome.
