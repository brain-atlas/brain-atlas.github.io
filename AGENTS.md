...
- Visibility: canonical lesson snapshots are authoritative in Lesson mode. Atlas Home owns one persistent complete canonical snapshot and full orbit/zoom/pan/filter control. **Explore this scene** creates a temporary scene-derived Atlas branch, projects it into the retained Viewer controls by stable entity ID, synchronizes the actual rendered camera before allowlisted command batches, and discards that branch on Return. **Back to atlas** restores the persistent global state; **Return to lesson** restores the exact stable lesson token. Lesson mode must disable the fieldset. None of these modes may create another canvas, context, transform, or filter system. Geometry/activity status and source detail belong in **Model & sources**; do not duplicate them as in-canvas badges, persistent stage rows, or global progress/status chrome.
...
- Development mode exposes `window.__view = { camera, controls, scene, THREE }` for renderer inspection and `window.__lesson` for plain lesson/navigation/controller state — see `skills/user.md`. Vite removes both guarded hooks from production builds; verify that before a public release.
