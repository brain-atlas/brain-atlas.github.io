...
| INV-35 | The strict project catalog exposes exactly the reviewed early-visual, association-bundle, robust qualifying-region, and unmapped-SWM inspectables with stable owner/fidelity/renderer bindings; selection-only landmarks do not enter canonical scene entity IDs. | catalog + tract-region mapping tests | Selection and future search reuse domain IDs without forking visibility state, while runtime records cannot drift from offline evidence. |
