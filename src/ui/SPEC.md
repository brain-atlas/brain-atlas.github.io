...
| FAIL-31 | A playing lesson view appears frozen after its camera settles | Model clock stopped, expected layer is hidden, stochastic events are temporarily sparse, teaching geometry is clipped/occluded, a transparent pathway writes depth over centerline points, stale dynamic-buffer bounds cull live points, or visibility/selection factors dim events and endpoint caps | Compare settled-stage frames with canonical playback, model clocks, draw ranges and position checksums, in-frame events, dynamic-point culling/depth state, endpoint proximity/cap opacity, and selected renderer groups before changing an activity engine. Correct the failing layer only. |
