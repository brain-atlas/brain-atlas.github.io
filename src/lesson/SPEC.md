...
- `public/data/entities.json` — stable scene and inspectable domain IDs for the original 45-region teaching/default and endpoint-filter boundary, plus curated anatomy and relationship evidence. Optional entity `atlasId` is a nonempty source identifier; current region IDs copy `regions.json`'s `area` field exactly, checked by `test/catalog.test.js`.
- `public/data/julich_regions.json` — complete checked 157-region source vocabulary and bilateral renderer bindings. Production catalog creation validates it and derives 112 additional canonical entities without extending endpoint selectors or inspectables.
...
| Export | Contract |
|---|---|
| `createLessonCatalog(entityManifest, fidelityManifest, fibreFilterManifest?, julichManifest?)` | Validates strict catalogs and returns frozen sorted entity/inspectable/fidelity/fibre-filter IDs, Atlas-default IDs, presets, selectors, and record lookups. The optional third input defaults to no named presets. Production supplies the checked four-preset manifest and complete Jülich manifest; its 112 additions enter canonical entity state while the original 45 alone remain Atlas defaults and region endpoint selectors. Inspectables derive fidelity only from their resolved owner entity; each authored undirected relationship receives one deterministic reciprocal inspector projection. |
...
| INV-15 | Every canonical snapshot v2 carries one validated endpoint-filter state. Named presets resolve from the strict small catalog; custom selectors resolve only to the original 45 project region IDs or explicit unknown/ambiguous classes; preset IDs cannot disagree with their frozen query. The complete Jülich visualization vocabulary does not silently broaden this versioned filter boundary. The filter describes unordered geometric endpoint membership and is independent of modeled activity direction. | catalog, scene-state, command, adapter, import, Explore, and reference-lesson tests | Lesson/Atlas/Return cannot inherit stale fibre subsets or reinterpret storage A/B as source/destination. |
| INV-16 | Production merges exactly 112 source-bound Jülich additions into the 45-region canonical base. Additions are bilateral, hidden from Atlas defaults, and bound to the existing `fidelity.julich-regions` record and renderer path. | complete-catalog, Explore-default, and browser loading tests | Optional visualization expands vocabulary without eager geometry, duplicate state, or changed lesson/filter results. |
...
| FAIL-13 | Complete Jülich catalog identity, count, bilateral labels, status split, renderer binding, or hierarchy-path shape drifts | Checked source/runtime catalog mismatch | Reject startup before lesson use; regenerate from pinned sources rather than filtering or fuzzy-matching at runtime. |
...
| Add an entity | Add a stable prefixed ID and renderer binding; reference an existing reviewed fidelity record or add one from traceability evidence. Complete Jülich visualization entities belong in `julich_regions.json`; endpoint-filter eligibility still requires an explicit versioned endpoint change. | INV-4, INV-8, INV-16 |
...
| INV-16, FAIL-13 | `test/catalog.test.js`, `test/explore-session.test.js`, `test/region-catalog.test.js`, `test/asset-pipeline.test.js`, and browser request/interaction checks. |
