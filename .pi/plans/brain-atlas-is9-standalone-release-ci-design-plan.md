...
**Status:** Merged; draft-aware first-nightly recovery implemented locally and pending integration
**Date:** 2026-07-23
**Branch:** `fix/is9-nightly-draft-discovery`
...
- [ ] Pull requests, `main` pushes, `v*` tag pushes, and manual dispatches run locked Node install/audit and publication checks, standalone production staging, CGO-free Go tests, race/vet checks, six cross-builds, archive validation, and a Linux moved-binary smoke test. The complete Node suite remains a required local Nix gate under `brain-atlas-ek3`. *(PR and main events pass; `v*` and manual events remain unexercised.)*
...
- [ ] A successful current-`main` run refreshes exactly one non-latest prerelease named/tagged `nightly`; assets identify their source commit, remote SHA-256 digests are checked, stale runs cannot promote, and cleanup deletes only superseded nightly assets after the new set is usable. *(Two main build/artifact runs passed. The first publication stopped before mutation on an ambient-ref parsing error; the second created and fully uploaded one safe draft, then stopped because a published-only REST lookup could not rediscover it. Draft-aware recovery is implemented locally; successful publication remains pending.)*
...
- There are no published GitHub Releases or Git tags. One unpublished `nightly` draft from main run `29974211439` contains the complete eight-asset bundle for `f361d69506b84b90c4ae38635d33f241020e92c0`; it has no Git ref and must be resumed rather than deleted or clobbered.
...
Fresh local evidence includes 201 passing Node tests; all CGO-free Go tests; race and vet checks; zero npm vulnerabilities; actionlint on both workflows; ordinary publication isolation; two byte-identical six-target release builds with exact archives/checksums/provenance; extracted native CGO-free root/data/model/404/405 smoke checks; three passing production Chromium scenarios; offline nightly/stable publication plans; and schema-valid Gemma behavioral/boundary reviews with zero findings. PR run `29973046121` exposed that seven scientific asset-regeneration tests are intentionally byte-exact to the recorded Darwin arm64/Nix environment; decision `brain-atlas-ek3` removes Node tests from the Ubuntu release job rather than claiming a partial cross-platform suite, while preserving the complete local gate. Follow-up run `29973354780` exposed that Ubuntu runners do not guarantee ripgrep; the workflow now uses standard `grep` for artifact inspection, including the ordinary-build guard that a missing `rg` could otherwise have silently bypassed inside an `if`. Final PR run `29973428141` passed, and its downloaded six-target artifact passed independent checksum, provenance, bundle, and native runtime verification. After approved squash merge `d5b0782`, main run `29973693205` passed the build, reproducibility, Linux smoke, and artifact-upload gates; nightly publication then failed before any tag/release mutation because the parser read the push ref name `main` as though it were a requested nightly tag. The tested repair ignores ambient `GITHUB_REF_NAME` for the fixed nightly channel while retaining it as the stable-tag fallback. Main run `29974211439` then passed all build/artifact gates and safely created a complete unpublished draft, but the publisher's raw REST by-tag reread returned 404 because that endpoint serves published releases only. Primary-source research and a read-only live query verified that GitHub CLI resolves the draft by GraphQL database ID. The local repair uses that draft-aware path, validates numeric asset IDs, handles both same-commit and stale complete drafts without moving a missing tag, and refuses a draft that conflicts with an existing tag. If the next remote nightly attempt fails, human direction requires stopping the incremental patch cycle and considering a clean-slate release design. Malformed DeepSeek candidates led to tested final-state race hardening; no confirmed or unresolved Critical/Important issue remains. Review artifacts are under `.pi/reviews/20260723-standalone-release-ci/`.

Human direction authorizes the narrow local draft-aware repair. Commit, push, PR creation, merge, main push, tag/release publication, repository-setting change, signing, and notarization remain separately gated. The Bead must remain open through the first successful GitHub nightly run; a stable tag rehearsal remains separately gated.
