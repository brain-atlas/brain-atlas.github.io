...
**Status:** Merged; first-nightly publication repair verified locally and pending integration
**Date:** 2026-07-23
...
- [ ] A successful current-`main` run refreshes exactly one non-latest prerelease named/tagged `nightly`; assets identify their source commit, remote SHA-256 digests are checked, stale runs cannot promote, and cleanup deletes only superseded nightly assets after the new set is usable. *(The first main build/artifact passed, but publication failed safely before mutation because `GITHUB_REF_NAME=main` was incorrectly treated as an explicit nightly tag. A regression-tested parser repair is pending integration.)*
