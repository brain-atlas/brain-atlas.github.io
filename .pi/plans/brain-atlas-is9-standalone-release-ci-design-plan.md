...
**Status:** Implemented and verified; stable `v*` publication remains separately gated
**Date:** 2026-07-23
**Branch:** `main`
...
- [x] Pull requests, `main` pushes, `v*` tag pushes, and manual dispatches route through locked Node install/audit and publication checks, standalone production staging, CGO-free Go tests, race/vet checks, six cross-builds, archive validation, and a Linux moved-binary smoke test. The complete Node suite remains a required local Nix gate under `brain-atlas-ek3`. *(PR and main events passed in GitHub; parsed workflow tests cover the shared `v*`/manual build path without creating an unauthorized tag.)*
- [x] Successful builds upload one clearly named Actions artifact containing six deterministic archives, sorted SHA-256 checksums, and exact commit/toolchain provenance, retained for 14 days.
...
- [x] A successful current-`main` run refreshes exactly one non-latest prerelease named/tagged `nightly`; assets identify their source commit, remote SHA-256 digests are checked, stale runs cannot promote, and cleanup deletes only superseded nightly assets after the new set is usable. *(Run `29975671138` resumed the verified draft, published it at `bf552a6`, removed only the old managed assets, and passed independent tag, metadata, digest, Actions-artifact byte-identity, provenance, and moved-binary checks.)*
...
- [x] A pushed `v*` tag must resolve to the workflow commit before publishing. First publication uses GitHub CLI's draft/upload/publish flow; a retry succeeds without mutation only if the tag, release metadata, asset names, sizes, and remote digests match. *(Focused state-machine tests, exact-bundle dry runs, and workflow gates pass; no remote test tag was created because stable publication is an operational release act requiring separate approval.)*
...
- Current workflows: `.github/workflows/deploy-pages.yml` builds the ordinary static Pages artifact, while `.github/workflows/standalone-binaries.yml` builds and packages the standalone matrix. Both use GitHub-owned full-SHA-pinned Actions, top-level `permissions: {}`, read-only build permissions, and isolated deployment/publication permissions.
...
- The public `nightly` prerelease and Git ref resolve to `bf552a6da75e39e93719e1c19d5613adc69e1ea2`. It is prerelease/non-latest and has exactly eight commit-scoped assets; no stable release exists yet.
- `scripts/build-standalone.mjs --site-only` stages the standalone production site once so the identical embedded tree is reused across all six targets; the ordinary host-build command retains its prior behavior.
- A tracked `internal/site/dist/.gitkeep` lets clean checkouts compile `go:embed all:dist`; generated staging remains ignored, and `internal/site.FS()` still rejects runtime staging without `index.html`.
...
4. Discover a published release or pending draft with draft-aware `gh release view`. Do not use GitHub's published-only REST by-tag endpoint as the existence check. Normalize the returned release/asset database IDs before any possible cleanup.
5. If nightly does not exist, create it as a draft prerelease with `--latest=false`, upload all assets, verify remote size/digest values, re-check current main, then publish it.
6. If nightly exists, upload only the new commit-scoped names without clobbering. Existing same-name assets are permitted only when their size/digest already match (idempotent retry); mismatches fail.
7. Verify every new remote asset digest and re-check current main immediately before promotion.
8. For a pending draft with no conflicting tag, publish it at the approved commit and let GitHub create the missing `nightly` ref. For an already published channel, force-move only `refs/tags/nightly` to the approved commit and update the release title/body/provenance. Retain prerelease and non-latest flags and verify the resulting release/tag state.
9. Delete only assets whose names match the project's nightly prefix but not the promoted source SHA. Never delete stable or unknown assets.
10. A failure before promotion leaves the old nightly canonical and any candidate assets visibly commit-scoped. A failure after tag movement leaves a complete validated new set available; retry repairs metadata/cleanup idempotently.
...
Implemented through approved PRs #6, #8, and #9. The final source state on `main` is `bf552a6da75e39e93719e1c19d5613adc69e1ea2`. Verification-driven refinements beyond the initial design include clean/dirty source-state provenance with hard publication rejection, symlink rejection for package inputs/downloaded assets/output parents, final tag/main/release rereads around GitHub state transitions, draft-aware GitHub CLI lookup, numeric release-asset ID validation, Python-cache handling for the clean-tree gate, and disabled Go dependency caching for the standard-library-only module.

Local and PR evidence includes 206 passing Node tests; all CGO-free Go tests; race and vet checks; zero npm vulnerabilities; actionlint on both workflows; ordinary/standalone publication isolation; two byte-identical six-target release builds with exact archives/checksums/provenance; extracted native CGO-free root/data/model/404/405 smoke checks; three passing production Chromium scenarios; focused nightly/stable publication tests and dry runs; and high-risk behavioral/boundary review with no findings. Decision `brain-atlas-ek3` keeps the complete Darwin arm64/Nix Node suite as a required local gate instead of claiming a partial Ubuntu suite.

GitHub main run `29975671138` passed build, reproducibility, Linux smoke, artifact upload, and rolling-nightly publication; Pages run `29975671114` passed and deployed. The publisher resumed draft database ID `358424022` without clobbering, published `nightly` at `bf552a6`, and removed only the old managed names after final verification. Independent closeout downloaded all eight assets, matched every server SHA-256 and every byte to the run artifact, validated clean-source provenance and the six-target bundle, and ran the moved darwin/arm64 executable with `CGO_ENABLED=0` through root/data/model/404/405/signal checks. The public Pages and nightly URLs returned 200.

The two pre-success nightly attempts failed safely and informed the final design: run `29973693205` stopped before mutation on ambient-ref parsing, and run `29974211439` retained a complete unpublished draft when the published-only REST by-tag lookup could not rediscover it. No data, scientific representation, lesson, source asset, dataset version, or citation changed. `DATA_LICENSES.md`, scientific traceability, tract provenance, lesson validation, `CITATION.cff`, and existing third-party notice content therefore require no further update.

Stable `v*` publication remains an intentional operational gate: creating the first version tag/release requires its own version decision, documentation reconciliation, and explicit approval. Signing, notarization, repository settings, branch/worktree cleanup, and release-channel redesign remain outside this plan.

## Closeout

Plan saved to: `.pi/plans/brain-atlas-is9-standalone-release-ci-design-plan.md`

Implementation and nightly integration are complete. Maintainers should use [`docs/RELEASES.md`](../../docs/RELEASES.md) for routine builds, nightly semantics, checksum verification, and the separately approved stable-release procedure.
