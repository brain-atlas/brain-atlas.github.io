...
updated after the fibre-space audit `brain-atlas-yum.5`, physical-distance
activity correction `brain-atlas-yum.10`, and complete Jülich catalog expansion
`brain-atlas-yum.14.2` (2026-09-06). This document
...
| Jülich region shells, including LGN and V1 | **Data-derived/derived.** All 157 base region pairs from the Jülich-Brain v3.0.3 categorical maximum-probability map (winner-take-all), meshed and simplified. | Real L/R labels and 314 meshes; every catalog record uses the source rule right label = left label + 1000. No runtime mirroring. | Shell boundaries locate atlas regions; fresnel styling emphasizes silhouettes. The established 45-region teaching projection is visible by default; 112 source-bound additions are optional. | The pinned builder consumes the exact MPM, licensed XML identities, and licensed partial hierarchy. It reproduces the unchanged 45-region `regions.json`, complete `julich_regions.json`, and all 314 OBJ meshes. The hierarchy has 155 path occurrences for 151 identities, including repeated leaves; six XML identities remain explicitly unresolved. Six GapMaps are labeled. Maximum-probability surfaces suppress probabilistic uncertainty; hierarchy is nonspatial terminology, while teaching streams and display colors for the original 45 remain viewer metadata. | `public/data/regions.json`; `public/data/julich_regions.json`; `public/data/regions/*.obj`; `tools/assets/regions.py`; `tools/assets/manifest.json`; Jülich source in `DATA_LICENSES.md` |
...
| Superficial white matter / short U-fibres | **Data-derived/derived.** 15,000 short fibres re-tracked from the exact HCP-1065 1 mm nonlinear ICBM152-2009a FIB using a 2009c seed derived from exact TemplateFlow GM/WM maps with identical qform/sform code 4 matrices; unrounded endpoints were filtered through a dilated cortical ribbon and contours were sampled to eight RAS-mm points. | Real bilateral tracking; assigned L/R by the sign of mean MNI x; no mirroring. | Static tangent grain plus analytically zero-mean along-contour vibration. One categorical endpoint query coherently filters grain and vibrating dots. The inspector exposes the layer with no relationship records. | The release-companion T1 indicates the FIB's asymmetric variant without a retained direct build binding. Checked preparation and recovered-intermediate post-processing reproduce the current JSON; manual DSI replay produced the required shape but different geometry and remains class 3. Every stored endpoint has a known, ambiguous, or unknown categorical assignment against the Jülich MPM. The seed/ribbon used whole-volume GM/WM maps, not a cerebral-only exclusion. A separate private carpet audit found broad non-cerebral/deep source-compartment encounters; it is not anatomical certification and changes no runtime classes (see `docs/TRACT_SPACE_PROVENANCE.md#swm-source-compartment-screening`). This mixed-2009a/2009c common-RAS comparison uses no template warp or voxel-grid equivalence and does not establish firing rate, histological U-fibre identity, biological terminations, region-to-region connections, polarity, or individual-axon bidirectionality. | `public/data/swm_fibres.json`; `public/data/fibre_endpoints.json`; `tools/assets/swm.py`; `tools/assets/endpoints.py`; `tools/assets/manifest.json`; `src/fibre-endpoint-filter.js`; `src/activity/swm-vibration.js`; `src/main.js` `loadSwm`/`updateSwm`; `docs/TRACT_REGION_MAPPING.md`; `docs/TRACT_SPACE_PROVENANCE.md`; decision `brain-atlas-cs0` |
