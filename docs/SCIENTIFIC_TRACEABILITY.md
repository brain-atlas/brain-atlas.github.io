...
| SWM/U-fibre vibration | Illustrative model driven partly by derived structure | One dot per fibre; home parameter 0.35–0.65; frequency 0.35–1.05 Hz; random phase; amplitude `clamp(0.5 × local_mean_length / own_length, 0.08, 0.45)`; contour parameter clipped to 0.03–0.97. | Communicates local short-fibre orientation and length variation without net travel. | Frequency, phase, scale factor, and bounds are display choices. The length ratio is derived structure, not neural activity. Clipping can distort a sinusoid near endpoints, so “zero mean” is a visual/no-net-travel intent rather than a guaranteed analytic mean for every current SWM sample. |
...
| SWM endpoint clipping can bias the claimed zero-mean vibration. | Weakens the direction-neutral representation guarantee. | `brain-atlas-yum.8`. |
