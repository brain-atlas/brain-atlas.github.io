...
| Named long association tracts | **Data-derived/derived.** Eight HCP-1065 population-averaged ICBM-2009a atlas bundles: ILF, IFOF, SLF I–III, VOF, arcuate, and MdLF. Each hemisphere contains 180 sampled fibres with 40 RAS-mm points. | Real bilateral atlas streamlines; no runtime mirroring. | Static coloured bundle density plus seeded inhibited code-like impulses in both modeled population directions. The inspector exposes a conservative subset of undirected, low-confidence endpoint-proximity links to displayed region shells. | The checked pinned generator consumes the stable release archive and reproduces current corrected output byte-for-byte. Its posterior-first contour reversal preserves a storage convention only; tractography contour order is not axonal polarity. Every event samples an explicit evidence-absent 50/50 A→B/B→A assumption. Offline endpoint proximity ignores order and requires at least 18/180 streamlines in each hemisphere at both 3 mm and 5 mm nearest-surface screens; counts are not terminations, connections, strengths, population probabilities, functions, or directions. | `public/data/tracts.json`; `public/data/tract_region_mapping.json`; `tools/map_tract_regions.py`; `tools/assets/association.py`; `tools/assets/manifest.json`; `public/data/tract_activity.json`; `src/activity/association-impulses.js`; `src/main.js` `loadTracts`/`updateTractImpulses`; `docs/TRACT_REGION_MAPPING.md`; `docs/TRACT_SPACE_PROVENANCE.md` |
| Superficial white matter / short U-fibres | **Data-derived/derived.** 15,000 short fibres re-tracked from the exact HCP-1065 1 mm nonlinear ICBM152-2009a FIB using a 2009c seed derived from exact TemplateFlow GM/WM maps with identical qform/sform code 4 matrices; unrounded endpoints were filtered through a dilated cortical ribbon and contours were sampled to eight RAS-mm points. | Real bilateral tracking; assigned L/R by the sign of mean MNI x; no mirroring. | Static tangent grain plus analytically zero-mean along-contour vibration. This remains the approved U-fibre representation; the inspector exposes the layer with no relationship records. | The release-companion T1 indicates the FIB's asymmetric variant without a retained direct build binding. Checked preparation reproduces the seed, and checked post-processing reproduces current JSON byte-for-byte from the recovered 200,000-fibre TrackVis intermediate, including `len` and `lloc`. Manual DSI replay produced 200,000 tracts and the required 15,000-fibre shape but different geometry and bytes, so it remains class 3 and cannot replace current data. No named-region endpoint classification is approved; vibration does not claim firing rate, histological U-fibre identity, region-to-region connections, or individual-axon bidirectionality. | `public/data/swm_fibres.json`; `tools/assets/swm.py`; `tools/assets/manifest.json`; `src/activity/swm-vibration.js`; `src/main.js` `loadSwm`/`updateSwm`; `docs/TRACT_REGION_MAPPING.md`; `docs/TRACT_SPACE_PROVENANCE.md`; decision `brain-atlas-cs0` |
...
| `layer.swm` | Explicit absence of an approved named-region mapping. | `fidelity.superficial-white-matter`; no relationship records. |
