...
After the WebGL gate, the renderer eagerly loads the cortical GLB, optic-radiation JSON, association geometry/activity data, compact endpoint tuples, and region manifest. Renderer readiness resolves after region and tract metadata plus endpoint tuples bind; a manifest or renderer failure returns the page to readable lesson fallback. The first canonical visibility snapshot then starts each required bilateral region OBJ pair and, when visible, the independently packaged SWM JSON. A direct checked lesson therefore starts with LGN, V1, V2, V3v, V3d, and the independently packaged SWM geometry because the opening snapshot uses the shared-early endpoint query. Later scenes still defer their additional regions. Atlas Home's complete default requests every region and SWM. The 2.4 MB association file remains eager because it combines panel metadata and geometry; splitting it requires a reviewed asset-pipeline change. [`PERFORMANCE.md`](PERFORMANCE.md) records production-profile evidence and device limits.
