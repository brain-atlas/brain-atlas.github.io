...
tract renderer IDs and camera presets. Its strict 33-record `inspectables` projection
...
After the WebGL gate, the renderer eagerly loads the cortical GLB, optic-radiation JSON, association geometry/activity data, and region manifest. Renderer readiness resolves after region and tract metadata bind; a manifest or renderer failure returns the page to readable lesson fallback. The first canonical visibility snapshot then starts each required bilateral region OBJ pair and, when visible, the independently packaged SWM JSON. A direct checked lesson therefore starts with LGN/V1 geometry and no SWM; later scenes request their own regions and SWM. Atlas Home's complete default requests every region and SWM. The 2.4 MB association file remains eager because it combines panel metadata and geometry; splitting it requires a reviewed asset-pipeline change. [`PERFORMANCE.md`](PERFORMANCE.md) records production-profile evidence and device limits.
